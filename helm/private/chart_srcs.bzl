...
_ATTRS = dicts.add({
        ...
        "force_repository_append": attr.bool(default = True, doc="""
            ...
            repository value. %s
            The repository value is read from the chart values.yaml; when the chart has no values.yaml file it is left untouched.
        """ % DEFAULT_VALUES_REPO_YAML),
        "path_to_chart": attr.string(doc="Workspace-relative path to the root directory of the chart. It is only used when `srcs` contains neither a Chart.yaml nor a values.yaml (otherwise the root is the directory of the outermost one), and it is mandatory in that case if `srcs` is not empty."),
        "_subst_template": attr.label(allow_single_file = True, default = ":substitute.sh.tpl"),
}, _DEPRECATED_ATTRS, STAMP_ATTRS)



# Chart layout is computed from `short_path` (workspace-relative, without the bazel-out prefix) so that
# files generated by other rules and checked-in sources of the same chart share a single chart root.
def _find_outer_file(files):
    file = None if len(files) == 0 else files[0]

    for f in files:
        if len(f.short_path) < len(file.short_path):
            file = f

    return file

# path of a chart file relative to the chart root, as it has to be laid out inside the packaged chart
def _chart_relative_path(file, chart_root, name=""):
    chart_root = chart_root.rstrip("/")
    ...
    if not file.short_path.startswith(prefix):
        fail("Chart %s: file %s is not placed under the chart root %s. Every chart src file has to live under the directory of the outermost Chart.yaml/values.yaml (or under path_to_chart)" % (name, file.short_path, chart_root))
...
def _locate_chart_roots(srcs, path_to_chart="", name=""):
    ...
    if manifest:
        root_path = paths.dirname(manifest.short_path)
    elif values:
        root_path = paths.dirname(_find_outer_file(values).short_path)
    else:
        root_path = path_to_chart.rstrip("/")

    # only a values.yaml placed at the chart root is the chart values file: values.yaml files of
    # vendored subcharts or environment specific ones are not
    value_file = None

    for value in values:
        if paths.dirname(value.short_path) == root_path:
            value_file = value
            break
...
def _chart_srcs_impl(ctx):
    ...
    for file in _filter_manifest_and_values_from_files(ctx.files.srcs):
        copied_file = ctx.actions.declare_file(paths.join(ctx.attr.name, chart_name, _chart_relative_path(file, chart_root_path, chart_name)))
    ...
    # the values file is read from its actual location: a generated values.yaml lives under bazel-out,
    # not next to the chart sources. An empty path makes the substitution script start from empty values.
    src_values_path = chart_files.values.path if chart_files.values else ""
    ...
    if ctx.attr.force_repository_append:
        ...
        if is_image_from_oci:
            if not _image_repository:
                # Add @sha256 suffix to the repository read from the chart values.yaml. Without a
                # values file there is nothing to read from (and yq called without a file argument
                # prints its usage), so the repository value is left untouched.
                if chart_files.values:
    ...
    for dep in chart_deps:
        ...
        for dep_src in dep.srcs:
            out_path = paths.join(ctx.attr.name, chart_name, "charts", dep.name, _chart_relative_path(dep_src, dep_chart_files.root, dep.name))
